...
function OrderCard({ order, role }) {
  ...
  return (
    <div
      ...
      className="flex items-center gap-4 px-4 py-3.5 bg-white border border-gray-100 rounded-xl mb-2 cursor-pointer hover:border-gray-200 transition-colors"
    >
      ...
      <div className="flex-1 min-w-0">
        <p className="text-sm font-medium text-gray-900 line-clamp-1">{order.listing?.title}</p>
        <div className="flex items-center gap-2 mt-1">
          <StatusBadge status={order.status} />
          <span className="text-xs text-gray-400">{formatShipping(order.shipping_method)}</span>
        </div>
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8">
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-400 mt-0.5">Track and manage your transactions</p>
      </div>
